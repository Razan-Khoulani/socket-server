...
const validateDriverLocationPoint = (driverId, lat, long, at = Date.now()) => {
  ...
  if (elapsedMs > 0) {
    const speedMps = distMeters / (elapsedMs / 1000);
    if (
      distMeters > LOCATION_MAX_JUMP_METERS &&
      elapsedMs < LOCATION_MAX_JUMP_WINDOW_MS
    ) {
      return { ok: false, reason: "jump", elapsedMs, distMeters, speedMps };
    }
    if (speedMps > LOCATION_MAX_SPEED_MPS) {
      return { ok: false, reason: "speed", elapsedMs, distMeters, speedMps };
    }
  }
...
console.log("[driver-online][meta-stored]", {
  driver_id: driverId,
  driver_service_id: baseMeta.driver_service_id ?? null,
  driver_detail_id: baseMeta.driver_detail_id ?? null,
  has_access_token: !!baseMeta.access_token,
});
...
const finalMeta = driverLocationService.getMeta(driverId) || {};
console.log("[driver-online][final-meta]", {
  driver_id: driverId,
  driver_service_id: finalMeta.driver_service_id ?? null,
  driver_detail_id:
    finalMeta.driver_detail_id ??
    finalMeta.driver_details_id ??
    null,
  has_access_token: !!finalMeta.access_token,
  service_type_id: finalMeta.service_type_id ?? null,
  service_category_id: finalMeta.service_category_id ?? null,
});
